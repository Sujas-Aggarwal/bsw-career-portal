...
export default function Hero({profileName}: {profileName: string|undefined}) {
    ...
    return (
        <div className="flex md:h-screen h-[calc(100vh-50px)]  relative overflow-hidden md:bg-transparent bg-[var(--theme-color)]">
            <Background bg_image={profile?.bg_image ? profile.bg_image : ""} />
            <div className="flex flex-col text-center justify-center items-center gap-[6px] text-white  md:pl-[10vw] p-2">
...
function Background({ bg_image }: { bg_image: string }) {
    ...
    return (
        <div className="hidden md:flex object-cover object-center">
            <img
                src={bg_image}
