    ...
    <div className="w-full  overflow-hidden pt-16 bg-white">
      <div className="w-full px-8 md:px-16 flex-col flex justify-center items-center h-full">
        <div className="flex flex-col items-center  gap-8 w-full">
          <h1 className="font-bold text-2xl md:text-3xl text-start w-full">
            Overview
          </h1>
          <p className="">{profile.overview}</p>
          <div className={`  ${readMore ? "block" : "hidden"}`}>
            <p dangerouslySetInnerHTML={{ __html: profile.readmore }} />
          </div>
          <button
            className="mt-2 mr-auto border-solid border-2 border-[var(--theme-color)] py-3 px-6 hover:bg-[var(--theme-color)] hover:text-white"
